...
const LeaderboardTable = ({
  ...
  return <div className="space-y-0">
      ...
      return <div key={player.user_id} className={`px-4 py-5 transition-all ${player.rank === 1 ? "bg-gradient-to-r from-gold/20 via-gold/10 to-transparent border-b-2 border-gold/30" : player.rank <= 3 ? "bg-muted/30 border-b-2 border-border/50" : ""} ${isCurrentUser ? "bg-orange/10 border-l-4 border-l-orange" : ""}`}>
            <div className="flex items-center gap-4">
              {/* Rank/Medal */}
              <div className="flex items-center justify-center w-12 h-12">
                {player.rank === 1 ? <div className="flex flex-col items-center">
                    <span className="text-3xl">{medal}</span>
                    <Trophy className="h-4 w-4 text-gold -mt-1" />
                  </div> : <span className="text-3xl">{medal}</span>}
              </div>
              ...
              <div className="flex items-center gap-3 flex-1">
                ...
                <div>
                  ...
                  {isCurrentUser && <span className="text-xs text-orange font-semibold">You</span>}
                </div>
              </div>
      ...
      const isCurrentUser = player.user_id === currentUserId;
      const flag = COUNTRY_FLAGS[player.country_code] || "🌐";
      return <div key={player.user_id} className={`px-4 py-4 border-b border-border/30 transition-all hover:bg-muted/20 ${isCurrentUser ? "bg-orange/10 border-l-4 border-l-orange sticky top-0 z-10" : ""}`}>
            <div className="flex items-center gap-4">
              {/* Rank */}
...
const EmptyState = () => {
  ...
  return <div className="text-center py-12 px-6 space-y-6">
      ...
      <Button onClick={() => navigate('/game')} size="lg" className="px-8">
        Start Today's Quiz
      </Button>
    </div>;
};
const LeaderboardSkeleton = () => {
  return <div className="space-y-0">
      {/* Top 3 skeleton */}
      {[1, 2, 3].map(i => <div key={i} className={`px-4 py-5 border-b-2 ${i === 1 ? 'border-gold/30' : 'border-border/50'}`}>
          <div className="flex items-center gap-4">
      ...
      {/* Rest skeleton */}
      {[4, 5, 6, 7, 8].map(i => <div key={i} className="px-4 py-4 border-b border-border/30">
          <div className="flex items-center gap-4">
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      const {
        ...
        body: {
          period: activeTab,
          country_code: countryFilter === "all" ? null : countryFilter,
          limit: 100
        }
      });
      ...
      <main className="container max-w-3xl mx-auto px-4 py-6 flex-1">
        <div className="space-y-6">
          ...
          <div className="bg-muted/50 p-1 rounded-xl border-2 border-border inline-flex w-full max-w-2xl mx-auto">
            {tabs.map(tab => <button key={tab.value} onClick={() => handleTabChange(tab.value)} className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${activeTab === tab.value ? "bg-background text-foreground shadow-sm border-2 border-border" : "text-muted-foreground hover:text-foreground"}`}>
                {tab.label}
              </button>)}
          </div>
          ...
          <Card className="overflow-hidden border-2 border-border bg-background rounded-xl">
            {isLoading ? <LeaderboardSkeleton /> : leaderboardData.length > 0 ? <LeaderboardTable data={leaderboardData} currentUserId={currentUserId || undefined} /> : <div className="p-6">
                <EmptyState />
              </div>}
          </Card>
        </div>
