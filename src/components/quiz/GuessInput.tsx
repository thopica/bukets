...
const GuessInput = ({ onGuess, onRequestHint, onShuffle, disabled = false, hintsRemaining = 0, currentHint, showError = false, showSuccess = false, hintsUsed = 0 }: GuessInputProps) => {
  ...
  return (
    <div className="p-1 md:p-3 md:bg-background md:border-t md:border-t-2 md:border-border md:sticky md:bottom-0 md:shadow-floating z-[1000]" style={{ paddingBottom: `max(0.25rem, env(safe-area-inset-bottom))` }}>
      <div className="space-y-1 md:space-y-2 md:container md:max-w-5xl md:mx-auto">
        ...
        <div className="flex gap-1 md:gap-2 items-center w-full">
          <Input
            ...
            disabled={disabled}
            className={`flex-1 min-w-0 h-8 md:h-12 text-sm md:text-sm text-foreground bg-card border md:border-2 rounded-md md:rounded-xl px-2 md:px-4 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-offset-0 transition-all duration-150 placeholder:text-muted-foreground shadow-elevated ${
              isShaking 
          ...
          {onRequestHint && (
            <Button
              ...
              variant="outline"
              size="icon"
              className={`h-7 w-7 md:h-12 md:w-12 rounded-md md:rounded-xl shrink-0 border md:border-2 transition-all relative ${
                hintsRemaining === 0 
            ...
            >
              <div className="flex flex-col items-center justify-center gap-0">
                ...
                <span className={`text-[6px] md:text-[8px] font-bold leading-none transition-colors ${
          ...
          {onShuffle && (
            <Button
              ...
              variant="outline"
              size="icon"
              className="h-7 w-7 md:h-12 md:w-12 rounded-md md:rounded-xl shrink-0 border md:border-2"
              title="Random quiz"
            >
          ...
          <Button
            ...
            size="icon"
            className="h-7 w-7 md:h-12 md:w-12 rounded-md md:rounded-xl shrink-0 font-bold"
          >
