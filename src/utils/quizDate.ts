...
export const getTodaysQuizIndex = (): number => {
  ...
  const quizIndex = daysPassed % quizzesData.quizzes.length;
...
export const getTodaysQuiz = (): Quiz => {
  ...
  return quizzesData.quizzes[index] as Quiz;
};

export const getQuizByIndex = (index: number): Quiz => {
  const safeIndex = Math.max(0, Math.min(index, quizzesData.quizzes.length - 1));
  return quizzesData.quizzes[safeIndex] as Quiz;
};

export const getTotalQuizzes = (): number => {
  return quizzesData.quizzes.length;
};
